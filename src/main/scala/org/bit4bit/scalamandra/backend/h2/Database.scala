...
class Database(dbname: String) extends org.bit4bit.scalamandra.backend.Database {
  val logger = LoggerFactory.getLogger(s"database:${dbname}")

  Class.forName("org.h2.Driver")
  ConnectionPool.add(dbname, s"jdbc:h2:mem:${dbname}", "user", "pass")

  //implicit val session = AutoSession

  def name(): String = s"jdbc:h2:mem:${dbname}"
  ...
  def create_table(table_name: String): Unit = {
    ...
    logger.info(query)
    NamedDB(dbname) localTx { implicit sessio =>
